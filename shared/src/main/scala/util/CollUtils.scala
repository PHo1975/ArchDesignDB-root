 ...
 */
object CollUtils {Log
  def iterHeadOption[T](iter:Iterator[T])= if(iter.hasNext)Some(iter.next()) else None
  ...
  def writePrimitive(out:DataOutput,data:Any)= {
    data match {
  ...
  def readOptString(in:DataInput)= if(in.readBoolean()) in.readUTF() else ""
