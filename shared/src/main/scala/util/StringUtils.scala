...
object StringUtils {
  val __OBFUSCATE = "OBF:"
   def stringToFloat(st:String)= try {
    val tt=st.trim
    if(tt.length==0)0f
    else tt.replace(',','.').toFloat
  } catch {  case e:Exception => 0f  }
  
  def stringToInt(st:String)= try {
    val tt=st.trim
    if(tt.length==0)0 
    else tt.toInt
  } catch {  case e:Exception => 0  }
  
  def stringToDouble(st:String)= try {
    val tt=st.trim
    if(tt.length==0)0d
  ...
  def mySgn(d:Double)= if(d<0) -1 else 1
  
  def fillTextLayout(g:Graphics2D,tl:TextLayout,xpos:Float,ypos:Float): Unit = {
  ...
  def encode(n:String)='"'+n.replace('"','«')+'"'

  def decode(n:String)=n.substring(1,n.length-1).replace('«','"')

  def escapeJSON(string:String)=
