	    ...
	    text.charAt(0) match {
	      case '$' => text.charAt(1) match {
	        ...
	        case 'I'|'i'=>
						...
						(new IntConstant(text.substring(2,end).toInt),end)
					case 'L'|'l'=>
						val end=CurrencyConstant.findEnd(text,2)
						(new LongConstant(text.substring(2,end).toLong),end)
					case 'N'|'n'=> (EMPTY_EX,2)
	        ...
	        case 'T'|'t'=> (new BoolConstant(text.charAt(2)=='1'),3)
	        case 'U'|'u'=> DateConstant.decode(text)case 'V'|'v'=> VectorConstant.decode(text)case 'Y'|'y'=> Variable.decode(text)
	      ...
	      case '"' =>
					...
					(new StringConstant(text.substring(1,pos).replace('«','"')),pos+1)
