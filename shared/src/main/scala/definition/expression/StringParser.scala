...
case class ParserError(_message: String, val offset: Int) extends ParserResult {
  lazy val message: String = { // translate
    if(_message==null)"null" else
    if (_message.contains("""string matching regex `\z""")) {
...
class StringParser extends JavaTokenParsers {
  ...
  val UnitElemMatcher="""([a-zA-ZäöüÄÖÜ\u00A3\u20AC]+)(\d*)""".r
  
  def unitElem: Parser[UnitElem]= UnitElemMatcher ^^ {y => y match {
    case UnitElemMatcher(s,n)=>new UnitElem(s,if(n.isEmpty())1 else n.toInt.toByte)
    case _=> new UnitElem(y,1)}}
  
  
  def unitList: Parser[List[UnitElem]] =
    ((unitElem ~ rep("·" ~> unitElem)) ^^ { case ex ~ list => ex :: list.map(_.asInstanceOf[UnitElem]) }) |
      (unitElem ^^ { case ex => List(ex) })

  def someNumber: Parser[String] = """-?(\d+[\.,]\d*|-?\d*[\.,]\d+|\d+)""".r

  def unitNumber: Parser[Expression] = (someNumber ~ unitList ~ "/" ~ unitList) ^^ { case num ~ nom ~ d ~ den => new UnitNumber(makeDouble(num), new UnitFraction(UnitNumber.setFactory ++ nom, UnitNumber.setFactory ++ den)) } |
    (someNumber ~ unitList) ^^ { case num ~ list => new UnitNumber(makeDouble(num), new UnitFraction(UnitNumber.setFactory ++ list, UnitNumber.emptySet)) }
  ...
  def nullParser: Parser[Expression] = """null""".r ^^ { case n => EMPTY_EX }
  ...
  def paramList: Parser[List[Expression]] =
    ((nonWhiteComp ~ rep(";" ~> nonWhiteComp)) ^^ { case ex ~ list => ex :: list.map(_.asInstanceOf[Expression]) }) |
      (nonWhiteComp ^^ { case ex => List(ex) })

  def currValue: Parser[Expression] =
    (someNumber ~ currencySymbols) ^^ { case a ~ c => CurrencyConstant(math.round(a.replace(',', '.').toDouble * 100L)) }

  def function: Parser[Expression] =
    (ident ~ ("(" ~> paramList <~ ")")) ^^ { case name ~ list => FunctionCall(None, name, list) }

  def trueVal: Parser[Expression] = "true|TRUE".r ^^ { case el => BoolConstant(true) }

  def falseVal: Parser[Expression] = "false|FALSE".r ^^ { case el => BoolConstant(false) }
  ...
  def makeDouble(st: String) = st.replace(',', '.').toDouble
}
    ...
    if (text.length == 0) Expression.generateNullConstant(expectedType)
      ...
      result match {
        ...
        case NoSuccess(err, next) => {
          if (expectedType == DataType.StringTyp) new StringConstant(text)
          else new ParserError(if (err == null) "Null" else err, next.offset)
        }
      }
    }
  }
