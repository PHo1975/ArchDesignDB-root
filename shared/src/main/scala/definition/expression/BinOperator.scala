...
abstract class BinOperator(val opChar: Char, val level: Byte) extends Serializable {
        ...
        case _ => c2 match {
          ...
          case _ => /* if(c1.getType==DataType.UnitNumberTyp||c2.getType==DataType.UnitNumberTyp)
		         getUnitValue(c1.toUnitNumber,c2.toUnitNumber)
                        ...
                        LongConstant(getLongValue(c1.toLong, c2.toLong))
                 else IntConstant(getLongValue(c1.toInt, c2.toInt).intValue)
        }
      }
    } else c1.getType match {
      case DataType.IntTyp => IntConstant(getLongValue(c1.toInt, c2.toInt).intValue)
      case DataType.LongTyp => LongConstant(getLongValue(c1.toLong, c2.toLong).longValue)
  ...
  override def toString: String = opChar.toString

  private[expression] def getUnitSafeValue(left: Constant, right: Constant, keepFirstValueUnit: Boolean) = try {
    getValue(left, right)
  } catch {
    case _: WrongUnitException => Some(new UnitNumber(getDoubleValue(left.toDouble, right.toDouble), (if (keepFirstValueUnit) left else right).toUnitNumber.unitFraction))
  }
}
...
object BinOperator {
  val plusOp: BinOperator = new BinOperator('+', 1) {
    ...
    def getUnitValue(left: UnitNumber, right: UnitNumber): UnitNumber = if (left.unitFraction.equals(right.unitFraction))
    ...
    def getValue(left: Constant, right: Constant): Some[Constant] = {
      if (left.isNumberConstant) Some(doCalculation(left, right))
      else left.getType match {
        case DataType.StringTyp => Some(StringConstant(left.toString + right.toString))
  ...
  val minusOp: BinOperator = new BinOperator('-', 1) {
    def getLongValue(left: Long, right: Long): Long = left - right
  ...
  private val oList = Map(
    ...
    '*' -> new BinOperator('*', 2) {
      ...
      def getUnitValue(left: UnitNumber, right: UnitNumber): UnitNumber = new UnitNumber(left.value * right.value, left.unitFraction.mult(right.unitFraction))

      def getValue(left: Constant, right: Constant): Some[Constant] = {
        if (left.isNumberConstant) Some(doCalculation(left, right))
        else
          left.getType match {
            case DataType.StringTyp => Some(StringConstant(multString(left.toString, right.toInt)))
    ...
    '/' -> new BinOperator('/', 2) {
      ...
      def getUnitValue(left: UnitNumber, right: UnitNumber): UnitNumber = new UnitNumber(left.value / right.value, left.unitFraction.div(right.unitFraction))
    ...
    '=' -> new CompBinOperator('=') {
      def getValue(left: Constant, right: Constant): Some[BoolConstant] = {
        left.getType match {
          case DataType.DoubleTyp | DataType.CurrencyTyp | DataType.UnitNumberTyp => Some(BoolConstant(left.getValue.toDouble == right.getValue.toDouble))
          case _ => Some(BoolConstant(left.getValue == right.getValue))
        }
    ...
    '<' -> new CompBinOperator('<') {
      def getValue(left: Constant, right: Constant): Some[BoolConstant] = {
        left.getType match {
          case DataType.StringTyp => Some(BoolConstant(left.getValue.toString < right.getValue.toString))
          case DataType.DoubleTyp | DataType.CurrencyTyp | DataType.UnitNumberTyp => Some(BoolConstant(left.getValue.toDouble < right.getValue.toDouble))
          case _ => Some(BoolConstant(left.getValue.toInt < right.getValue.toInt))
        }
      }
    },
    '>' -> new CompBinOperator('>') {
    ...
    '&' -> new CompBinOperator('&') {
      def getValue(left: Constant, right: Constant): Some[BoolConstant] = {
        Some(BoolConstant(left.toBoolean && right.toBoolean))
      }
    },
    '|' -> new CompBinOperator('|') {
      def getValue(left: Constant, right: Constant): Some[BoolConstant] = {
