...
case class CollectingFuncCall(name: String, propertyField: Byte, childType: Int, childField: Byte,
  ...
  override def createCopy(): Expression = {new CollectingFuncCall(name, propertyField, childType, childField, cacheValue)}
...
object CollectingFuncCall {
  def decode(text: String): (CollectingFuncCall, Int) = {
