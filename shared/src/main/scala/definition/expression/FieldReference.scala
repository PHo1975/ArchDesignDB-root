...
def createCopy(): Expression = new FieldReference(remType,remInst,remField,cachedValue)
...
def setCachedValue(newVal:Constant) = cachedValue=newVal
  ...
  def encode="$R("+(remType match {case Some(t)=>t.toString;case _=>""})+";"+
