...
case class FontStyle(styleName:String,fontName:String,height:Float,bold:Boolean,italic:Boolean,underline:Boolean,color:Color) {
  ...
  def javaStyle=(if(bold)Font.BOLD else 0) + (if(italic)Font.ITALIC else 0)
  def graphStyle=(if(bold)FontStyle.boldStyle else 0) + (if(italic)FontStyle.italicStyle else 0)
