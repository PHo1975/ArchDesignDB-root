...
case class DialogQuestion(name:String,possibleAnswers:Seq[AnswerDefinition],repeat:Boolean=false,visualOption:String="") extends ParamQuestion {
	...
	override def toString = "Question " +name+" answers="+possibleAnswers+" repeat:"+repeat+" visual:"+visualOption
	def classID=1
}
...
case class PanelRemoteQuestion(val panelClass:String) extends PanelQuestion {
...
case class XMLQuestion(moduleName:String, customData:Seq[scala.xml.Node]) extends ParamQuestion {
  def toXML ():scala.xml.Node = {
    <XMLQuestion moduleName={moduleName}  > {customData}  </XMLQuestion>
  }
  def repeat=false
  def classID=4
}
    ...
    if(dnode.isEmpty){
      ...
      if(dnode.isEmpty){
        ...
        if(dnode.isEmpty){
          ...
          else Some(new CommandQuestion((dnode \"@moduleName").text,(dnode \"@command").text))
        }
        else Some(new PanelRemoteQuestion((dnode \"@name").text))
      }
      else Some(new XMLQuestion((dnode \"@moduleName").text,dnode.head.child))
    }
    else {
      ...
      Some(new DialogQuestion(name,answers,repeat,visual))
    }
  }

  def fromStream(in:DataInput):ParamQuestion = {
    in.readInt() match {
      case 1=> new DialogQuestion(in.readUTF,for(i<-0 until in.readInt) yield AnswerDefinition.fromStream(in),in.readBoolean(),in.readUTF())
      case 2=> new PanelRemoteQuestion(in.readUTF)
      case 3=> new CommandQuestion(in.readUTF,in.readUTF)
      case 4=> new XMLQuestion(in.readUTF,null)
    }
  }
