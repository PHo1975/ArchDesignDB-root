...
import util.XMLUtils.{readBool,readOptString,optText,boolText}
...
case class FieldSetting (fieldNr:Int,var readOnly:Boolean=false,
  ...
  // formatField == this field will be used to format a graphical element in the graph editor
	override def toString =
  	 ...
  	 (if(showFormula)" showFormula " else "")+(if(editor.size>0)" editor:"+editor else "")+" startValue:"+startValue+"formString:"+formString+"formField:"+formatField+"]"
